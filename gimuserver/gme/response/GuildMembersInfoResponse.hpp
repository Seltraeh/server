...
RESPONSE_NS_BEGIN
struct GuildMembersInfoResponse : public IResponse
{
	const char* getGroupName() const override { return "csIuech30"; }
		...
		uint32_t m_CpPoints = 0;
		std::string m_LastOnline = "";
		uint32_t m_Tokens = 0;
		uint32_t m_ActiveGuildDeck = 0;
...
protected:
	void SerializeFields(Json::Value& v, size_t) const override {
			...
			v["9SVsdnwl"] = std::to_string(m_CpPoints);
			v["93nclsc8"] = m_LastOnline;
			v["23b6d2ia"] = std::to_string(m_Tokens);
			v["Z0Y4RoD7"] = std::to_string(m_ActiveGuildDeck);
	}
};
RESPONSE_NS_END
