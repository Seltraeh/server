...
RESPONSE_NS_BEGIN
struct ChallengeArenaOpponentUnitInfoResponse : public IResponse
{
	const char* getGroupName() const override { return "zyJhDEiF"; }

		uint32_t m_gr48vsdJ = 0;
		uint32_t m_4HIy6A1Q = 0;
		std::string m_UserID = "";
...
protected:
	void SerializeFields(Json::Value& v, size_t) const override {
			v["gr48vsdJ"] = std::to_string(m_gr48vsdJ);
			v["4HIy6A1Q"] = std::to_string(m_4HIy6A1Q);
			v["h7eY3sAK"] = m_UserID;
			...
			v["iNy0ZU5M"] = std::to_string(m_Element);
			v["oS3kTZ2W"] = m_LeaderSkillID;
			...
			v["5gXxT7LZ"] = std::to_string(m_ExtCnt);
			v["0R3qTPK9"] = std::to_string(m_NewFlg);
	}
};
RESPONSE_NS_END
