...
RESPONSE_NS_BEGIN
struct UserUnitInfoResponse : public IResponse
{
	const char* getGroupName() const override { return "qC2tJs4E"; }
	...
	struct Data {
		...
		uint32_t m_NewFlg = 0;
		uint32_t m_DbMVG16I = 0;
		std::string m_AddExtraPassiveSkillID = "";
		uint32_t m_UnitImgType = 0;
		uint32_t m_FeBP = 0;
		uint32_t m_FeUsedBP = 0;
		uint32_t m_FeMaxUsableBP = 0;
	};
...
protected:
	void SerializeFields(Json::Value& v, size_t) const override {
		...
		for (const auto& data : items) {
			...
			item["ISj9u5VL"] = std::to_string(data.m_LimitOverHp);
			...
			item["D6bKH5eV"] = std::to_string(data.m_LimitOverAtk);
			...
			item["3CsiQA0h"] = std::to_string(data.m_LimitOverDef);
			...
			item["XJs2rPx0"] = std::to_string(data.m_LimitOverHeal);
			...
			item["DbMVG16I"] = std::to_string(data.m_DbMVG16I);
			item["cP83zNsv"] = data.m_AddExtraPassiveSkillID;
			item["2pAyFjmZ"] = std::to_string(data.m_UnitImgType);
			item["bFQbZh3x"] = std::to_string(data.m_FeBP);
			item["3RgneFpP"] = std::to_string(data.m_FeUsedBP);
			item["GIO9DTif"] = std::to_string(data.m_FeMaxUsableBP);
			arr.append(item);
		}
